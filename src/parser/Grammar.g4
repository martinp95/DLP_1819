...
sentencia returns[Sentencia ast]
	...
	| expr op=('+' | '-' | '*' | '/') '=' expr ';' { $ast = new Asignacion($ctx.expr(0), new ExprAritmetica($ctx.expr(0), $op, $ctx.expr(1))); }
	| expr '&&=' expr ';' { $ast = new Asignacion($ctx.expr(0), new ExprLogica($ctx.expr(0), "&&", $ctx.expr(1)));}
	| expr '||=' expr ';' { $ast = new Asignacion($ctx.expr(0), new ExprLogica($ctx.expr(0), "||", $ctx.expr(1)));}										
